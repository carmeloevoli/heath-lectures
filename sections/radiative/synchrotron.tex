...
\TODO{add the full derivation from the covariant expression of Larmor?}
...
Notice that at \emphit{fixed particle energy} \(E=\gamma mc^{2}\), one has \(\gamma \propto 1/m\), hence \(P_{\rm s}\propto \gamma^{2}/m^{2}\propto m^{-4}\). This steep mass dependence makes synchrotron losses overwhelmingly important for electrons and positrons compared to nuclei.
Moreover, it is important to remark that the energy loss is \emphit{proportional to the magnetic field energy density \( U_B \)}.
...
\label{eq:tau-loss}
\end{equation}
Thus~\emphit{higher-energy} electrons cool~\emphit{faster}, a key driver of spectral steepening in synchrotron-dominated sources.
...
\]
For light (\(u'=c\)) one obtains the standard \emphit{aberration formula}:
\begin{equation}
...
In the non-relativistic limit the emission is essentially mono-chromatic at the gyrofrequency \(\nu_0\), because the charge executes uniform circular motion and the radiation pattern is broad. 

\TODO{add a plot with a(t) and frequency}
...
\begin{equation}\label{eq:nuc-full}
...
\item for back-of-the-envelope estimates, the \emphit{delta approximation} collapses the spectrum to \(P_\nu\simeq P_{\rm s}\,\delta(\nu-\nu_c)\) and reproduces the correct scalings with \(B\), \(\gamma\), and \(\theta\); 
\item the explicit \(\sin\theta\) reminds us that synchrotron is powered by the \emphit{perpendicular} acceleration: no pitch angle, no radiation.
\end{itemize}

\begin{figure}[t]
\centering
\includegraphics[width=0.65\textwidth]{synchrotron_kernel_Fx.pdf}
\caption{The synchrotron kernel $F(x)$ describing the synchrotron spectrum emitted by the single electron. Exact $F(x)=x\!\int_x^\infty K_{5/3}$ (solid) with low-/high-$x$ asymptotes and the heuristic $1.8\,x^{1/3}e^{-x}$.}
\label{fig:synchro_kernel}
\end{figure}

\section{Synchrotron emission by an electron population.}

In most sources the radiating electrons span a broad range of energies. The most common modelization of the parent electron population is a power–law in Lorentz factor,
\[
n(\gamma)\,d\gamma = n_0\,\gamma^{-p}\,d\gamma,
\qquad
\gamma_{\min}<\gamma<\gamma_{\max},
\]
where $n$ is the differential number density (L$^{-3}$) and the typical slopes in many astrophysical environments ranges in $p\simeq2$–$3$ . 
%
The specific emissivity (power per unit volume and frequency) follows by summing the single–electron spectra over the population and pitch angles,
\[
...
\[
P_\nu(\gamma,\theta)\;\simeq\;\langle P_{\rm s}(\gamma)\rangle \; \delta\!\big(\nu-\nu_c(\gamma,\theta)\big),
\]
with \( \langle P_{\rm s}(\gamma)\rangle \) as in~\ref{eq:Ps-avg}, and \( \nu_c(\gamma,\theta) \) as in~\ref{eq:nuc-full}.

%\[
%\langle P_{\rm s}(\gamma)\rangle=\frac{4}{3}\,c\,\sigma_{\rm T}\,U_B\,\gamma^{2},
%\qquad
%\nu_c(\gamma,\theta)=\frac{3}{2}\,\gamma^2\,\nu_0\,\sin\theta,
%\qquad
%\nu_0\equiv \frac{qB}{2\pi m_e c},
%\]
%and $U_B=B^2/(8\pi)$ as before.  

Inserting the $\delta$–function, and using the identity
%
\begin{equation}
\delta\big(\nu-\nu_c(\gamma)\big)
= \frac{\delta\big(\gamma-\gamma_\nu\big)}{\big|\frac{d\nu_c}{d\gamma}\big|_{\gamma_\nu}},
\qquad
\gamma_\nu \;\equiv\; \sqrt{\frac{\nu}{\tfrac{3}{2}\nu_0\sin\theta}},
\end{equation}
%
...
\begin{equation}
j_\nu
= \int_{\gamma_{\min}}^{\gamma_{\max}}\!\!n_0\,\gamma^{-p}\,
\langle P_{\rm s}(\gamma)\rangle\,
\delta\big(\nu-\nu_c(\gamma)\big)\,d\gamma 
%&\simeq n_0\,
%\frac{\langle P_{\rm s}(\gamma_\nu)\rangle\,\gamma_\nu^{-p}}
%     {\left|\dfrac{d\nu_c}{d\gamma}\right|_{\gamma_\nu}} \\
= n_0\,
\frac{\tfrac{4}{3}c\sigma_{\rm T}U_B\,\gamma_\nu^{2-p}}
     {2\,(\tfrac{3}{2}\nu_0\sin\theta)\,\gamma_\nu} 
%&\propto U_B\,\gamma_\nu^{1-p}\,\nu_0^{-1}
%\;\propto\;
%U_B\,\nu^{-\frac{p-1}{2}}\,
...
Thus we recover the \emphit{synchrotron law}:
%
\begin{tcolorbox}
\begin{equation}
...
In conclusion, an electron index $p$ produces a synchrotron index 
\[
\alpha\equiv -\frac{d\ln j_\nu}{d\ln\nu}=\frac{p-1}{2}.  
\]
...
\bottomrule
\end{tabular}
\caption{Electron and synchrotron slopes for cooling–dominated steady state (assuming no escape).} 
\label{tab:source-to-spectrum}
\end{table}

\TODO{Add example for Galaxy synchrotron spectrum: frequency vs energy / slope}

\TODO{say something about polarization}
