...
\TODO{ADD HERE THE COSMIC RAY PLOT.}

\begin{table}[t]
  \centering
  \begin{tabular}{lccc}
    \hline
    Band & Wavelength $\lambda$ [m] & Frequency $\nu$ [Hz] & Energy $E$ [eV] \\
    \hline
    Radio           & $\sim 1$ -- $10^{4}$      
                    & $\sim 10^{4}$ -- $3\times 10^{8}$      
                    & $\sim 4\times 10^{-11}$ -- $10^{-6}$ \\[2pt]
    Microwaves      & $\sim 10^{-3}$ -- $1$            
                    & $\sim 3\times 10^{8}$ -- $3\times 10^{11}$ 
                    & $\sim 10^{-6}$ -- $10^{-3}$ \\[2pt]
    Infrared (IR)   & $\sim 10^{-6}$ -- $10^{-3}$      
                    & $\sim 3\times 10^{11}$ -- $4\times 10^{14}$ 
                    & $\sim 10^{-3}$ -- $2$ \\[2pt]
    Visible (optical) & $\sim 4\times 10^{-7}$ -- $7\times 10^{-7}$ 
                      & $\sim 4\times 10^{14}$ -- $7.5\times 10^{14}$ 
                      & $\sim 1.6$ -- $3$ \\[2pt]
    Ultraviolet (UV) & $\sim 10^{-8}$ -- $4\times 10^{-7}$ 
                     & $\sim 7.5\times 10^{14}$ -- $3\times 10^{16}$ 
                     & $\sim 3$ -- $10^{2}$ \\[2pt]
    X-rays          & $\sim 10^{-11}$ -- $10^{-8}$     
                    & $\sim 3\times 10^{16}$ -- $3\times 10^{19}$ 
                    & $\sim 10^{2}$ -- $10^{5}$ \\[2pt]
    Gamma rays      & $\lesssim 10^{-11}$              
                    & $\gtrsim 3\times 10^{19}$        
                    & $\gtrsim 10^{5}$ \\
    \hline
  \end{tabular}
  \caption{Approximate ranges of the electromagnetic spectrum in wavelength,
  frequency, and photon energy. Boundaries between bands are conventional and
...
We begin with a minimal model to show how continuous energy losses lead to a kinetic (continuity) equation for the particle distribution, such that the steady state differs from the injected spectrum, producing observable spectral imprints that diagnose regions of particle acceleration. 

\TODO{CITE LONGAIR HERE}

Let \(N(E,t)\) be the differential \emphit{number density} per unit energy (\(\mathrm{L^{-3}\,E^{-1}}\)), and \(Q(E,t)\) the \emphit{injection rate density} (\(\mathrm{L^{-3}\,T^{-1}\,E^{-1}}\)). Energy losses are 
\[
...
\end{equation}
\end{tcolorbox}
This is the continuity equation in energy space (with no sources/sink yet): the term \(\partial_E[bN]\) is the energy–space flux that \emphit{advects} particles from high to low energy. The origin of the \(\partial_E b\) piece is the Jacobian \(\Delta E'/\Delta E = 1 + (\partial_E b)\Delta t\).

%\paragraph{Adding sources and sinks.}
Injection adds \(Q(E,t)\,\Delta E\,\Delta t\) particles to the bin, while escape removes \(\left[N(E,t) \tau_{\rm esc}^{-1}(E)\right]\Delta E\,\Delta t\).
...
\paragraph{Monochromatic injection}
For continuous, \emphit{monoenergetic injection} \(Q(E)=Q_0\,\delta(E-E_{\rm inj})\) and no escape, equation~\eqref{eq:ss_noescape} gives
%
\begin{equation}
...
For a \emphit{power–law injection} \(Q(E)=Q_0 E^{-p}\) (\(p>1\)) and losses \(b(E)=b_0 E^{m}\) (\(m>0\)), equation~\eqref{eq:ss_noescape} gives
%
...
\paragraph{The cooling break.}

If particles are continuously injected for a time $t_*$ \emphit{without} radiative losses, the spectrum simply accumulates:
\begin{equation}
...
\end{cases}
\]
Thus the spectral index steepens by $\Delta \alpha = m -1$ across the break; in particular, for synchrotron/IC in the Thomson regime ($m=2$) the steepening is $+1$. Locating $E_{\rm c}$ is therefore highly diagnostic: it ties together the source age (or escape time) and the strength of the target fields that set $b(E)$. 

\TODO{Add exercise for Crab.} %, letting us to get an estimate of the age of an object from the identification of the break energy.
...
\end{equation}
with the \emphit{cooling cutoff}:
\begin{tcolorbox}
\begin{equation}
...
%\]
no initial energy $E_i$ exists that could have cooled to $E$ by time $t$ ($\rightarrow$ the inversion $E_i(E,t)$ has no solution), hence $N(E,t)=0$ for $E\ge E_{\rm cut}(t)$. Under \emphit{continuous} injection, this sharp cutoff is replaced by a \emphit{cooling break}; for a \emphit{single burst} it remains a time-dependent high-energy cutoff at $E_{\rm cut}(t)$.

\TODO{say more about cooling spike}
...
%In steady state with no escape, the \emphit{master formula} is 
...
%The cooling break marks the transition where $t_{\rm cool}(E)=E/b(E)$ becomes the shortest timescale: 
