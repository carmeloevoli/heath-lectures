...
\centering
\includegraphics[width=0.6\textwidth]{figures/halo.pdf}
\caption{Illustrative depiction of the Milky Way's diffusive halo from the perspective of a cosmic-ray physicist. Primary sources are distributed in the thin gaseous disc, while secondary nuclei are mainly produced by cosmic-ray interactions within the same region.}
\label{fig:galaxy}
\end{figure}

The local density of CRs, their energy spectrum, and their relative abundances provide the only \emph{direct} information we have about Galactic cosmic rays. These measurements, together with estimates of the CR column density obtained from diffuse \(\gamma\)-ray emission~\cite{Tibaldo2021universe, Grenier2015araa}, form the empirical foundation for any model of CR transport.
%
In this section our goal is to construct a minimal, yet instructive, model of CR propagation in the Galaxy, and to show how such a model allows us to extract from the data quantitative information about CR injection and their subsequent processing in the ISM.

As discussed in Section~\ref{sec:pillar}, the escape time inferred from secondary nuclei and radioactive isotopes is far too long to be compatible with straight-line propagation along large-scale magnetic-field lines. Instead, CRs must be \emph{confined} in the Galaxy for a long time and spend most of it in regions of low gas density, such as the Galactic halo.
%
The existence of such an extended, magnetized halo --- thicker than the gaseous disc --- is supported by observations of synchrotron emission from edge-on external galaxies~\cite{Beck2015aar}.

To capture these essential features, essentially all phenomenological CR transport models rely on a common picture (see Fig.~\ref{fig:galaxy}):
%
\begin{itemize}
\item High-energy CRs are accelerated in sources located in a thin disc of half-thickness \(h \sim 100\)~pc, with an injected spectrum \(\propto E^{-\gamma}\), where \(\gamma \gtrsim 2\), as expected for diffusive shock acceleration.
\item After injection, CRs propagate diffusively in a halo of typical half-height
\(H \sim \text{a few kpc}\), with a diffusion coefficient scaling as \(D(E) \propto E^\delta\), where \(\delta \sim 1/3\text{--}1/2\).
\item At the halo boundaries \(z = \pm H\) CRs escape freely into intergalactic space. In practice, this escape is modeled by setting the CR density to zero at the boundaries~\cite{Ginzburg1980apss}.
\end{itemize}
%
Since the halo size \(H\) is much smaller than the Galactic radius \(R_{\rm d} \gtrsim 10\)~kpc, a one-dimensional (1D) description along the vertical, \(z\)-direction is usually sufficient to describe CR escape.

\paragraph{Diffusion equation and Green’s function.}

The simplest mathematical implementation of this framework is based on the diffusion equation in the form of Fick’s law~\cite{Fick1855}. One may also start from the more general Fokker–Planck equation for pitch-angle scattering and then reduce it to a diffusion equation in configuration space under suitable assumptions~\cite{Mertschreview?}. In both cases, the resulting transport equation can be viewed as a phenomenological closure: one specifies the form of the flux entering the fundamental continuity equation.

In 1D, the continuity equation with a source term \(Q(E,z)\) reads
%
\begin{equation}
\frac{\partial n}{\partial t} + \frac{\partial J}{\partial z} = Q,
\end{equation}
%
where \(n(E,z,t)\) is the CR number density per unit energy, and \(J(E,z,t)\) is the corresponding flux along \(z\).

According to Fick’s law of diffusion, the diffusive flux is proportional to the gradient of the density,
\[
J = -D \,\frac{\partial n}{\partial z}.
\]
A spatial gradient in the particle density thus gives rise to a current that transports particles from regions of high density to low density, with a proportionality constant given by the diffusion coefficient \(D(E,z)\). The dimensions of \(D\) are \emph{area per unit time}.

Substituting this closure into the continuity equation yields the diffusion equation
%
\begin{equation}
\frac{\partial n}{\partial t} =
\frac{\partial}{\partial z} \left( D \frac{\partial n}{\partial z} \right) + Q.
\end{equation}

For a spatially constant diffusion coefficient, \(D = \text{const}\), one can derive the associated Green’s function,
%
\begin{equation}
\mathcal G(z,t) = \left(\frac{1}{4\pi D t}\right)^{1/2}
\exp\!\left(-\frac{z^2}{4Dt}\right),
\label{eq:green}
\end{equation}
%
which we interpret as the probability density of finding a particle at position \(z\) at time \(t\), given that it was injected at \(z = 0\) at \(t = 0\).

\begin{figure}[t]
\centering
\includegraphics[width=0.6\textwidth]{figures/gaussian_1D.pdf} 
\caption{1D Gaussian propagator as a function of distance for various time steps. The plot illustrates the progressive broadening of the diffusion Green’s function, Eq.~\eqref{eq:green}, with increasing time.}
\label{fig:gaussian1d}
\end{figure}

From Eq.~\eqref{eq:green} we can compute the mean square distance from the Galactic plane,
%
\begin{equation}
\langle z^2 \rangle =
\int_{-\infty}^{+\infty} dz\, z^2\, \mathcal G(z,t)
= 2 D t,
\end{equation}
%
so that the root-mean-square (rms) displacement is
\[
\sqrt{\langle z^2 \rangle} = \sqrt{2Dt}.
\]
Up to numerical factors of order unity, which we will not keep track of in what follows, this shows that the typical distance reached by a diffusing particle scales as
\[
z_{\rm diff}(t) \sim \sqrt{D t}.
\]

The characteristic time needed to reach a height of order the halo size, \(z_{\rm diff} \sim H\), is therefore
\[
t_H \sim \frac{H^2}{D},
\]
and we can associate to this a characteristic ``drift'' or effective escape speed
%
\begin{equation}
v_{\rm D} \sim \frac{H}{t_H} \sim \frac{D}{H}.
\end{equation}
%
This \(v_{\rm D}\) should not be confused with a physical bulk velocity; it simply parametrizes how quickly diffusion fills the halo on average.

It is important to stress that, in order to obtain these simple relations, we assumed a diffusion coefficient that is spatially constant throughout the disc and halo. In reality, both the gas density and the magnetic-field strength vary between these regions, and the diffusion properties are likely to be different in the disc and in the halo. Nevertheless, the constant-\(D\) model provides a useful baseline and captures the correct scaling of typical distances and times with \(D\) and \(H\).

In the remainder of this chapter, we will adopt this phenomenological diffusive framework as our starting point and build the CR evolution equation by specifying the source term for Galactic supernovae (SNe), which we take as the dominant contributors to the population of Galactic CR protons.
...
\paragraph{Transport equation for protons.}

Since the Galactic disc is extremely thin compared to the halo, we can describe the spatial part of the injection term as a delta function, \(\delta(z)\), and write
%
\begin{equation}\label{eq:qprotons}
Q(E,z) = 
{\color{Red}\underset{\text{[\text{E}]}}{\xi_{\rm CR} E_{\rm SN}}} \,
{\color{ForestGreen}\underset{[\text{T}^{-1}]}{\mathcal R_{\rm SN}}} \,
{\color{RoyalBlue}\underset{[\text{L}^{-3}]}{\frac{\delta(z)}{\pi R_{\rm d}^2}}} \, 
N(E)~,
\end{equation}
%
where \(E_{\rm SN} \simeq 10^{51}\)~erg is the kinetic energy released in a typical supernova explosion, \(\xi_{\rm CR}\) is the fraction of this energy converted into CRs, \(\mathcal R_{\rm SN} \simeq 1/50~\text{yr}^{-1}\) is the Galactic SN rate, and \(N(E)\) is the energy spectrum injected by a single SN (normalized to unit energy).

The age of the Galaxy is of order several Gyr, whereas the CR residence time inferred from radioactive isotopes is at most \(\sim 10^8\)~yr. Hence the global Galactic properties evolve on timescales much longer than those relevant for CR transport. It is therefore justified, as a first approximation, to assume a \emph{stationary} configuration. In steady state, the diffusion equation for protons, \(n_p(E,z)\), becomes
%
\begin{equation}
-\,\frac{\partial}{\partial z}\left[ D(E)\,\frac{\partial n_p}{\partial z}\right]
= \frac{\xi_{\rm CR} E_{\rm SN} \mathcal R_{\rm SN}}{\pi R_{\rm d}^2}\,N(E)\,\delta(z).
\label{eq:protons}
\end{equation}

For \(z \neq 0\) the right-hand side vanishes, and with the boundary conditions \(n_p(z = \pm H,E) = 0\) the solution can be written as\footnote{The general solution for \(z \neq 0\) is \(n_p(z)=A+Bz\). For \(z>0\), imposing \(n_p(H)=0\) gives \(n_p(z)=A\left(1- z/H\right)\); for \(z<0\), imposing \(n_p(-H)=0\) gives \(n_p(z)=A\left(1+ z/H\right)\). Matching the two regions and imposing \(n_p(0)=n_0(E)\) yields Eq.~\eqref{eq:nzwithabs}.}
%
\begin{equation}
D\,\frac{\partial n_p}{\partial z} = \text{const}
\quad\longrightarrow\quad
n_p(z) = n_0(E)\,\left( 1 - \frac{|z|}{H} \right),
\label{eq:nzwithabs}
\end{equation}
%
where \(n_0(E)\) is the proton density at the disc, \(z=0\).

The associated diffusive flux is then independent of \(z\), and we can evaluate it just above the disc,
%
\begin{equation}
\left. D\,\frac{\partial n_p}{\partial z}\right|_{z=0^+}
= -\,D\,\frac{n_0}{H}.
\label{eq:flux}
\end{equation}

To determine \(n_0(E)\) we integrate the diffusion equation across an infinitesimal interval around the disc:
%
\begin{equation}
\lim_{\epsilon\rightarrow 0} \int_{-\epsilon}^{+\epsilon} \!\! dz\,
\left\{
-\,\frac{\partial}{\partial z}\left[ D\,\frac{\partial n_p}{\partial z}\right]
\right\}
=
\lim_{\epsilon\rightarrow 0} \int_{-\epsilon}^{+\epsilon} \!\! dz\, Q(E,z),
\end{equation}
%
which leads to
%
\begin{equation}
\left. -\,2D\,\frac{\partial n_p}{\partial z}\right|_{z=0^+}
= \frac{\xi_{\rm CR} E_{\rm SN} \mathcal R_{\rm SN}}{\pi R_{\rm d}^2}\,N(E).
\end{equation}
%
Using Eq.~\eqref{eq:flux} for the flux at \(z=0\), we obtain
%
\begin{equation}
n_p(E) = n_0(E)
= \frac{\xi_{\rm CR} E_{\rm SN} \mathcal R_{\rm SN} N(E)}{2 \pi R_{\rm d}^2}\,\frac{H}{D(E)}.
\label{eq:protonsimplesolution}
\end{equation}

We have thus obtained the stationary CR proton spectrum as measured by an observer located in the disc. It can be rewritten in the more transparent form
%
\begin{tcolorbox}
\begin{equation}
n_p(E) = \frac{Q_{\rm SN}(E)\,\tau_{\rm esc}(E)}{V_{\rm G}},
...
\end{tcolorbox}
%
where \(Q_{\rm SN}(E) = \xi_{\rm CR} E_{\rm SN} \mathcal R_{\rm SN} N(E)\) is the total number of particles injected per unit time by all SNe in the Galaxy, \(\tau_{\rm esc} = H^2/D(E)\) is the characteristic escape time from the halo, and \(V_{\rm G} = 2\pi R_{\rm d}^2 H\) is the effective volume of the CR halo.

\begin{figure}[t]
\centering
\includegraphics[width=0.6\textwidth]{figures/diffusion_softening.pdf}
\caption{Comparison between the \emph{injection spectrum} and the \emph{propagated spectrum} in an energy-dependent \emph{diffusion} model. Higher-energy particles escape more rapidly, leading to a reduced equilibrium intensity. As a result, the propagated spectrum is systematically \emph{steeper} than the injected one.}
\label{fig:softening}
\end{figure}

Since the diffusion coefficient increases with energy, typically following a power law \(D(E) \propto E^{\delta}\), the escape time scales as \(\tau_{\rm esc}(E) \propto E^{-\delta}\). Consequently, the observed spectrum is always \emph{softer} than the spectrum injected at the sources.

More explicitly, if the sources inject a power-law spectrum \(N(E) \propto E^{-\gamma}\), then
\[
n_p(E) \propto N(E)\,\tau_{\rm esc}(E) \propto E^{-\gamma-\delta}.
\]
High-energy particles spend less time in the Galaxy than low-energy ones, so their equilibrium density is more strongly suppressed. This behavior is illustrated in Fig.~\ref{fig:softening}.

It is important to emphasize that what we observe is always the combined outcome of \emph{injection} and \emph{transport}. Any deviation from a pure power law in the observed spectrum of protons (or any other primary species) is therefore, in general, ambiguous: it may reflect features in the source spectrum or in the energy dependence of the diffusion coefficient. % , or in additional processes such as reacceleration, convection or energy losses.

Finally, it is interesting to make some remarks on the assumptions we have made so far.  

%Finally, let us briefly comment on the assumptions made so far:
%\begin{itemize}
%\item We treated the Galaxy as a 1D slab with homogeneous diffusion (\(D = D(E)\)) and sharp absorbing boundaries at \(z = \pm H\).
%\item We neglected all processes other than diffusion and escape: no energy losses, no spallation, no convection, no reacceleration.
%\item We assumed a stationary state, justified by the separation between the CR residence time and the evolutionary timescales of the Galaxy.
%\item We modeled sources as continuously distributed in a thin disc and described their vertical profile with \(\delta(z)\).
%\end{itemize}
%Despite these simplifications, the model already captures two crucial aspects of CR physics: (i) diffusion inevitably softens the propagated spectrum relative to the injection spectrum, and (ii) the normalization of the observed spectrum encodes the combination \(Q_{\rm SN} \tau_{\rm esc}/V_{\rm G}\), linking CR data to the SN population and the size of the diffusive halo. In the next sections we will progressively relax some of these assumptions and introduce additional processes, such as spallation and energy losses, to build a more realistic description of Galactic CR transport.
...
\paragraph{On the physics of boundary conditions.}

The physics of CR transport is influenced not only by diffusion, but also by the choice of boundary conditions.
%
As discussed earlier, the condition of free escape, implemented as \(n(z = \pm H, E) = 0\), implies that in steady state the diffusion current is independent of position.

At the halo boundary, conservation of particle flux requires
%
\begin{equation}
\left. D\,\frac{\partial n}{\partial z} \right|_{z=H}
= \frac{c}{2}\,n_{\rm out},
\end{equation}
%
where we have assumed that particles outside the diffusive volume stream away approximately at the speed of light, \(c\).\footnote{The flux through a semi-infinite plane is
\(\Phi = \int_0^1 d\mu\, \mu\, v\,n \simeq n\,c \int_0^1 d\mu\,\mu = (c/2)\,n\),
where \(\mu\) is the cosine of the pitch angle.}

On the other hand, we can express the diffusion coefficient in terms of the scattering mean free path \(\lambda\) as
\(D = \tfrac{1}{3} c \lambda\)\footnote{A simple derivation of this expression is given in Sec.~3.2 of Ref.~\cite{Kachelriess2008arxiv}.}.
Using the steady-state solution \(n(z) = n_0(1-|z|/H)\), we have
\(\partial n/\partial z|_{z=0^+} = -n_0/H\) and hence
\(D\,\partial n/\partial z|_{z=0^+} = -D n_0/H\).
Equating the magnitude of this diffusive flux to the outward streaming flux at the boundary gives
%
\begin{equation}
n_{\rm out} = \frac{2D}{cH}\,n_0 \sim \frac{\lambda(E)}{H}\,n_0.
\end{equation}

The condition of \emph{free escape}, \(n_{\rm out} \ll n_0\), is therefore satisfied as long as the mean free path is much smaller than the halo size,
\(\lambda(E) \ll H\).
For Galactic CRs this remains true up to energies well below the PeV range, so the diffusive approximation with absorbing boundaries is self-consistent at GeV--TeV energies.

Despite the central role of this assumption, we still lack a first-principles understanding of what sets the halo size \(H\), and whether \(H\) might depend on energy or position in the Galaxy. Some attempts to model the halo on physical grounds can be found in Refs.~\cite{Evoli2018prl,Dogiel2020apj}, but this remains an open problem.

\paragraph{Relativistic constraints on the diffusion equation.}

The diffusion equation admits solutions where the density becomes non-zero at arbitrarily large distances even for arbitrarily small times, as evident from the Green’s function in Eq.~\eqref{eq:green}. Physically, this corresponds to information about the source propagating instantaneously to all points in space, in apparent violation of the relativistic limit set by the speed of light, \(c\).

This issue arises because the diffusion equation is based on a Fickian closure in which the flux is proportional to the gradient,
\(\mathbf{J} = -D \nabla n\), with no built-in limit on the signal speed. The equation is parabolic, and its solutions develop infinitely extended ``tails'' instantly.

When using the standard diffusion equation, it is therefore important to ensure that the timescales involved are such that this unphysical, superluminal behavior is not dynamically relevant. For transport over a characteristic distance \(R\), the diffusion timescale is
\[
t_{\rm diff} \sim \frac{R^2}{D}.
\]
The effective propagation speed over this distance is
\[
v_{\rm diff} \sim \frac{R}{t_{\rm diff}} \sim \frac{D}{R}.
\]
Requiring this speed not to exceed the speed of light, \(v_{\rm diff} \lesssim c\), we obtain
\[
\frac{D}{R} \lesssim c \quad \Longrightarrow \quad D \lesssim c\,R.
\]
In other words, for a given spatial scale \(R\) the diffusion coefficient must not be so large that the diffusion front would advance faster than \(c\). In Galactic CR applications, with typical \(R\sim\) kpc and \(D\) in the range inferred from B/C, this condition is safely satisfied.

To address the superluminal tails of the diffusion equation more rigorously, relativistic generalizations have been proposed. A commonly discussed modification is based on the \emph{telegraph equation}, which incorporates a finite propagation speed for disturbances. In its simplest form it reads
\[
\tau\,\frac{\partial^2 n}{\partial t^2}
+ \frac{\partial n}{\partial t}
= D\,\nabla^2 n,
\]
where \(\tau\) is a characteristic relaxation time related to the microscopic scattering process. Unlike the standard diffusion equation, the telegraph equation is hyperbolic and supports wave-like solutions with a finite signal speed \(\sim \sqrt{D/\tau}\), thus restoring causality.

For ultra-high-energy cosmic rays (UHECRs), with energies \(\gtrsim 10^{18}~\text{eV}\), a naive non-relativistic diffusion description can become problematic, especially when propagation distances are comparable to or smaller than the scattering mean free path. In such regimes, relativistic transport models --- see e.g.\ Ref.~\cite{Berezinskii2012} --- treat the finite particle velocity explicitly and provide solutions that respect the speed-of-light constraint.
%
In practice, however, these more accurate descriptions are mathematically more involved, and for most Galactic CR applications the standard diffusion equation remains an excellent approximation.

\paragraph{The relevance of inelastic processes for protons.}

In deriving the proton transport equation so far, we have neglected inelastic nuclear losses. For CR protons, the dominant inelastic process in the ISM is inelastic \(pp\) scattering leading to pion production, characterized by a typical cross section
\(\sigma_{\rm pp} \sim 3\times 10^{-26}~\text{cm}^2\).

The corresponding interaction timescale in the ISM can be estimated as
%
\begin{equation}
\tau_{\rm pp} \simeq \frac{1}{\bar{n}\,\sigma_{\rm pp}\,c},
\end{equation}
%
where \(\bar{n} = n_{\rm d}\,h/H\) is the average gas density in the diffusive volume, with
\(n_{\rm d} \sim 1~\text{cm}^{-3}\) the typical mid-plane density and \(h/H\) accounting for the fact that gas is concentrated in the thin disc. For typical Galactic parameters, one finds
\(\tau_{\rm pp}\) of order \(\sim 10^9\)~yr, i.e. a Gigayear timescale.

Since this is much longer than the CR residence time in the halo, \(\tau_H \sim 10^7\text{--}10^8\)~yr, inelastic \(pp\) interactions do not significantly modify the propagated proton spectrum at GeV--TeV energies. It is therefore a good approximation to neglect these losses in the proton transport equation, at least as far as the \emph{proton spectrum} is concerned.

However, even if subdominant in terms of energy losses, inelastic \(pp\) scattering is crucial for high-energy \(\gamma\)-ray production: \(pp\) collisions produce neutral pions which promptly decay into two \(\gamma\)-rays. This process is the leading mechanism behind the spectacular diffuse emission observed along the Galactic plane by Fermi-LAT above \(\sim 100\)~MeV (see Section~\ref{sec:nucleiproton}).

For heavy nuclei, the situation is different. The relevant inelastic cross sections grow with mass number and become comparable to --- or shorter than --- the escape timescale; see Eq.~\ref{eq:sgeo}. In that case, nuclear fragmentation and inelastic destruction must be explicitly included in the transport equation. We will return to this point in Section~\ref{sec:nuclei}, where we discuss the propagation of CR nuclei through the ISM.
