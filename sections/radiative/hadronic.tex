...
\sigma_{\rm geom} \simeq \pi (1.2 A^{1/3})^2~\text{fm}^2 \simeq 45 \, A^{2/3}~\text{mb},
\end{equation}
where \(1~\text{mb} = 10^{-27}~\text{cm}^2\). In this simple picture, larger nuclei (larger \(A\)) correspond to larger effective areas and therefore larger interaction cross-sections: heavier nuclei are more likely to undergo hadronic interactions. Although real cross-sections deviate from this pure geometric estimate, Eq.~\eqref{eq:sgeo} provides a remarkably useful rule of thumb for nuclear interactions in astrophysics~\cite{Letaw1983apjs}. 

\medskip

The same geometric intuition carries over to interaction timescales. The \emph{interaction timescale} is the average time a high-energy particle travels before undergoing a collision with targets of number density \(n_{\rm target}\). For a particle moving at speed \(\simeq c\), one finds
\begin{equation}
\tau \simeq \frac{1}{n_{\rm target} \, \sigma_{\rm geom} \, c}.
\end{equation}
...
A particularly important class of hadronic processes for high-energy astrophysics is \emph{pion production} in proton--proton collisions. When a high-energy cosmic-ray proton hits a proton at rest in the interstellar medium, the available energy in the center-of-mass frame can be sufficient to create new particles. The lightest of these are the pions (\(\pi\)), and their production provides the main channel for generating high-energy gamma rays and neutrinos in hadronic scenarios.
...
\left[ 1 - \left( \frac{E^{\rm th}_p}{E_p} \right)^4 \right]^2 \, \text{mb},
\end{equation}
where 
\[
L = \ln\!\left(\frac{E_p}{\text{TeV}}\right),
\]
and \(E^{\rm th}_p\) is the threshold energy derived above. This expression smoothly interpolates between the rapid onset of inelasticity near threshold and the slow logarithmic growth of the cross-section at very high energies, and will be our workhorse in computing hadronic gamma-ray and neutrino yields in the following sections.
